...
// Adafruit_TinyUSB_MIDI_Class constructor
Adafruit_TinyUSB_MIDI_Class::Adafruit_TinyUSB_MIDI_Class(uint8_t n_cables) : _midi(n_cables) {}
...
Adafruit_USBD_MIDI& Adafruit_TinyUSB_MIDI_Class::getMidiInstance() {
    return _midi;
}
...
void Adafruit_TinyUSB_MIDI_Class::sendTimeCodeQuarterFrame(uint8_t typeNibble, uint8_t valuesNibble) {
    uint8_t packet[4] = {static_cast<uint8_t>(0x02), static_cast<uint8_t>(0xF1), static_cast<uint8_t>((typeNibble << 4) | (valuesNibble & 0x0F)), 0};
    _midi.writePacket(packet);
}

// Song Position
void Adafruit_TinyUSB_MIDI_Class::sendSongPosition(uint16_t beats) {
...
// Song Select
void Adafruit_TinyUSB_MIDI_Class::sendSongSelect(uint8_t songNumber) {
    uint8_t packet[4] = {static_cast<uint8_t>(0x02), static_cast<uint8_t>(0xF3), songNumber, 0};
    _midi.writePacket(packet);
}

// Tune Request
void Adafruit_TinyUSB_MIDI_Class::sendTuneRequest() {
    uint8_t packet[4] = {static_cast<uint8_t>(0x01), static_cast<uint8_t>(0xF6), 0, 0};
    _midi.writePacket(packet);
}

// Real-Time Messages
void Adafruit_TinyUSB_MIDI_Class::sendRealTime(uint8_t realTimeType) {
